...
class Membership(models.Model):
    ...
    def is_membership_family(cls, family: Family) -> bool:
        return Membership.objects.filter(family=family, academic_course=EstablishedCourse.load()).exists()

    @classmethod
    def is_membership_child(cls, child: Child) -> bool:
        return Membership.objects.filter(family=child.family, academic_course=EstablishedCourse.load()).exists()
