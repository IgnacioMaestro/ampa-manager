...
class ImportAfterSchoolsActivitiesViewTest(TestCase):
    URL = reverse('import_after_schools_activities')
    ...
    def test_import_after_schools_activities_post_valid_form(self, mock_import_after_school: MagicMock):
        ...
        self.assertEqual(
            response.context['excel_columns'], get_excel_columns(AfterSchoolsActivitiesImporter.COLUMNS_TO_IMPORT))
        self.assertEqual(response.context['form_action'], self.FORM_ACTION)
        self.assertEqual(response.context['form'].errors, {})
        self.assertEqual(type(response.context['form']), ImportAfterSchoolsActivitiesForm)
        self.assertEqual(response.context['form'].data, {})
        self.assertEqual(response.context['form'].files['file'].name, example_file.name)

    def test_import_after_schools_activities_post_invalid_form(self):
        # Act
    ...
    def test_import_after_schools_activities_get(self):
        # Act
    ...
    def test_import_after_schools_activities_put(self):
        # Act
